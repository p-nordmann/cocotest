...
def test_marks_cli_option_run_skipped():
    # Control
    code, output = _invoke("testbench/failures/test_skip.py", "-m", "skip")
    ...
    # Test
    code, output = _invoke(
        "testbench/failures/test_skip.py", "-m", "skip", "--run-skipped"
    )

    assert "SKIP" not in output
    assert "testbench/failures/test_skip.py::test_python_error_skipped: FAIL" in output
    assert code == 1
